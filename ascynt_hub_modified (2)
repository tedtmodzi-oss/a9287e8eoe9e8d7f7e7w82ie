...
local HotkeyBindings = {
    ["Open/Close Menu"] = "RightShift",
    ["Telekinesis"] = "T",
    ["Teleport to Island"] = "I",
    ["AntiFling"] = "F",
    ["Teleport to Safe Spawn"] = "G",
}
...
applyBorder = function(parent, color, thickness)
...
local function updateHotkeyButton(action)
    ...
    local keyName = binding and tostring(binding) or "Unassigned"
...
HotkeyInputConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if HotkeyCaptureAction then
        ...
        HotkeyBindings[action] = input.KeyCode.Name
